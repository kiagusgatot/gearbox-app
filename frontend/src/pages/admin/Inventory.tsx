...
export default function AdminInventory() {
  ...
  async function load() {
    setIsLoading(true)
    await Promise.all([api.get('/admin/parts').then(r=>setParts(r.data.data||[])), api.get('/admin/parts/low-stock').then(r=>setLowStock(r.data.data||[]))])
    setIsLoading(false)
  }
  useEffect(()=>{load()},[])
  ...
  async function handleRestock(e: React.FormEvent) {
    e.preventDefault(); if (!restockPart) return; setSubmitting(true)
  ...
  return (
    <AdminLayout>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28 }}>
          <div><div className="page-eyebrow">Manajemen</div><h1 className="page-title">Inventory</h1></div>
          <button className="btn-primary" onClick={()=>setAddModal(true)}><Plus size={14}/>Tambah Part</button>
        </div>
        {lowStock.length>0&&(
          <div style={{ background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:8, padding:'11px 18px', marginBottom:18, display:'flex', alignItems:'center', gap:10 }}>
            ...
            <p style={{ fontSize:13, color:'#92400E' }}><strong>{lowStock.length} suku cadang</strong> stoknya di bawah batas minimum.</p>
          </div>
        )}
        <div className="card" style={{ overflow:'hidden' }}>
          <div className="table-wrapper">
            <table>
              <thead><tr><th>SKU</th><th>Nama</th><th>Kategori</th><th>Harga</th><th>Stok</th><th>Min.</th><th>Status</th><th>Aksi</th></tr></thead>
              <tbody>
                {isLoading ? <tr><td colSpan={8} style={{ textAlign:'center', color:'#9CA3AF', padding:'32px 0' }}>Memuat...</td></tr>
                : parts.length===0 ? <tr><td colSpan={8} style={{ textAlign:'center', color:'#9CA3AF', padding:'32px 0' }}>Belum ada suku cadang</td></tr>
                : parts.map(p=>(
                  <tr key={p.id}>
                    ...
                    <td style={{ fontWeight:600, color:p.stock<=p.min_stock?'#DC2626':'#111827' }}>{p.stock} <span style={{ fontSize:11, fontWeight:400, color:'#9CA3AF' }}>{p.unit}</span></td>
                    <td>{p.min_stock}</td>
                    <td>{p.stock<=p.min_stock?<span className="badge badge-rejected" style={{ gap:3 }}><AlertTriangle size={10}/>Low Stock</span>:<span className="badge badge-verified">Aman</span>}</td>
                    <td><div style={{ display:'flex', gap:6 }}>
                      <button className="btn-secondary" style={{ padding:'5px 10px', fontSize:11, display:'flex', alignItems:'center', gap:4 }} onClick={()=>{ setRestockPart(p); setRestockQty(10); setRestockModal(true) }}>
                        <RefreshCw size={12}/>Restock
                      </button>
                      <button className="btn-danger" style={{ padding:'5px 10px' }} onClick={()=>setDeleteTarget(p)}><Trash2 size={13}/></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal open={restockModal} onClose={()=>setRestockModal(false)} title="Restock Stok" width={400}>
        <form onSubmit={handleRestock} style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div>
            ...
            {restockPart&&<p style={{ fontSize:11, color:'#9CA3AF', marginTop:5 }}>Stok setelah restock: <strong style={{ color:'#065F46' }}>{restockPart.stock+restockQty} {restockPart.unit}</strong></p>}
          </div>
          <div style={{ display:'flex', gap:8, justifyContent:'flex-end', paddingTop:8, borderTop:'1px solid #F3F4F6' }}>
            <button type="button" className="btn-secondary" onClick={()=>setRestockModal(false)}>Batal</button>
            <button type="submit" className="btn-primary" disabled={submitting}>{submitting?'Memproses...':'Tambah Stok'}</button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDialog
        ...
        variant="danger" title="Hapus Suku Cadang?"
